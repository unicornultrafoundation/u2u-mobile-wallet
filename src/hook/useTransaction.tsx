...
export const useTransaction = () => {
  ...
  const estimateGasLimit = async (txObject?: Record<string, any>) => {
    const internalTx = {
      ...
      gasPrice: (await estimateGasPrice()).toString(),
      data: txStore.txData
    }
  ...
  const estimatedFee = useMemo(() => {
    return BigNumber(txStore.estimatedGasLimit).multipliedBy(txStore.estimatedGasPrice).dividedBy(10 ** 18).toString()
  }, [txStore.estimatedGasLimit, txStore.estimatedGasPrice])

  const maxFee = useMemo(() => {
    return BigNumber(txStore.gasLimit).multipliedBy(txStore.gasPrice).dividedBy(10 ** 18).toString()
