...
export default function ConversationRow({item}: {
  ...
  const renderLastMessage = (item: Record<string, any>) => {
    const [messageCode, userID] = item.lastMessageContent.split(" ")
    switch (messageCode) {
      case '10':
        return t('acceptInvitation', {
          userWallet: truncate(userID, 10),
          dateString: isToday(item.updatedAt) ? formatDate(item.updatedAt, 'HH:mm') : formatDate(item.updatedAt, 'dd/MM HH:mm')
        })
      case '11':
        return t('rejectInvitation', {
          userWallet: truncate(userID, 10),
          dateString: isToday(item.updatedAt) ? formatDate(item.updatedAt, 'HH:mm') : formatDate(item.updatedAt, 'dd/MM HH:mm')
        })
      default:
        return truncate(item.lastMessageContent, 40)
    }
  }
    ...
    >
      ...
      <View style={{flex: 1, justifyContent: 'space-between', gap: item.role === 'pending' ? 12 : 0}}>
        <Text
          ...
          color="title"
        >
          {otherProfile ? `${truncate(otherProfile.name || otherProfile.id, 20)}` : shortenAddress(otherAddress, 12, 12)}
        </Text>
        {item.role === 'pending' ? (
