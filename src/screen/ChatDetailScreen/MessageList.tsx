...
import { formatNumberString, truncate } from "@/util/string"
...
export default function MessageList() {
  ...
  const renderSystemMessage = (item: Record<string, any>, index: number) => {
    const fromMe = item.from === wallet.address.toLowerCase()
    const [messageCode, userID] = item.content.split(" ")
    switch (messageCode) {
      case '10':
        return (
          <View
            style={{width: '100%', padding: 12, alignItems: 'center', justifyContent: 'center'}}
          >
            <Text
              type="footnote-small"
            >
              {
                t('acceptInvitation', {
                  ...
                  dateString: isToday(item.createdAt) ? formatDate(item.createdAt, 'HH:mm') : formatDate(item.createdAt, 'dd/MM HH:mm')
                })
              }
            </Text>
          </View>
